...
const load=():Project[]=>{try{return JSON.parse(localStorage.getItem(key)||'[]');}catch{throw new Error('无法读取演示记录，请检查浏览器是否允许本地存储。');}};
...
export const previewApi={
 ...
 async action(id:string,body:Record<string,unknown>):Promise<Project>{
  ...
  event(p,`演示步骤：${String(body.action)}`);return persist(p);
 }
};
